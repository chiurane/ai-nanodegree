...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        a_nodes = list()   # A list of our action nodes based on the actions to be extracted.

        # According to (1) above we must extract the actions to add to the level and create those PNode_a objects
        for action in self.all_actions:
            # First we create an Action Node for this Action. (1)
            # Note, according to Russell0-Novig, each Action at level Ai is connected to to its preconditions at Si
            # and effects its effects at Si+1.
            a_node = PgNode_a(action)
            # Test if to see if our proposed PgNode_a has prenodes that are a subset of the previous S level.
            if a_node.prenodes.issubset(self.s_levels[level]):
                a_nodes.append(a_node)
                # Second, this action need to be connected to Si and Si+1 nodes (child and parent) (2)
                for s_node in self.s_levels[level]:
                    s_node.children.add(a_node)
                    a_node.parents.add(s_node)

        # Now add the level here
    ...
    def add_literal_level(self, level):
        ...
        s_nodes = set() # we are working with sets, which dont add duplicates.
        # Note, An S node comes from a an a_node on the previous level
        for a_node in self.a_levels[level-1]:
            # Each a_node in the previous level has a list of S nodes in effnodes that represent the effects produced
            # by the action
            for s_node in a_node.effnodes:
                s_nodes.add(s_node) # Add to our set() here
                s_node.parents.add(a_node) # Give it the a_node as the parent
                a_node.children.add(s_node) # Give it children
        # Now we add the S-ith level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effects = [effect for effect in node_a1.action.effect_add if effect in node_a2.action.effect_rem] + \
                  [effect for effect in node_a2.action.effect_add if effect in node_a1.action.effect_rem]
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effects = [effect for effect in node_a1.action.precond_pos
                   if effect in node_a2.action.effect_rem or effect in node_a2.action.effect_add] + \
                  [effect for effect in node_a1.action.precond_neg
                   if effect in node_a2.action.effect_add or effect in node_a2.action.effect_rem] + \
                  [effect for effect in node_a2.action.precond_pos
                   if effect in node_a1.action.effect_add or effect in node_a1.action.effect_rem] + \
                  [effect for effect in node_a2.action.precond_neg
                   if effect in node_a1.action.effect_add or effect in node_a2.action.effect_rem]
        return len(effects) > 0
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effects = [(a1, a2) for a1 in node_a1.parents for a2 in node_a2.parents if a1.is_mutex(a2)]
        return len(effects) > 0
    ...
    def h_levelsum(self) -> int:
        ...
        # According to the AIMA book, the level sum is the sum of the level costs of the goals.
        for goal in self.problem.goal:
            ...
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    ...
                    if not state.is_pos:
                        literal = expr('~{}'.format(state.symbol)) # Cater for both negative and positive goals
                    if goal == literal:
